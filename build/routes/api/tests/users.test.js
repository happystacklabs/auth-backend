...
describe('users', function () {
  ...
  describe('POST: /users', function () {
    describe('success', function () {
      ...
      it('should save the user to database if request is valid', function (done) {
        _User2.default.count({}, function (err, count) {
          expect(count).toBe(0);
          (0, _supertest2.default)(_app2.default).post('/api/users').send(payload).expect(200).end(function (err, res) {
            _User2.default.count({}, function (err, count) {
              expect(count).toBe(1);
              done();
            });
    ...
    describe('username', function () {
      it('should return error if less than 5 characters', function (done) {
        ...
        (0, _supertest2.default)(_app2.default).post('/api/users').send(payload).expect(422).end(function (err, res) {
          _User2.default.count({}, function (err, count) {
            expect(count).toBe(0);
  ...
  describe('POST: /users/login', function () {
    beforeEach(function (done) {
      ...
      (0, _supertest2.default)(_app2.default).post('/api/users').send(payload).expect(200).end(function () {
        done();
      });
    });
    ...
    it('should return a JSON user representation if request is valid', function (done) {
      ...
      (0, _supertest2.default)(_app2.default).post('/api/users/login').send(payload).expect(200).end(function (err, res) {
        expect(res.body.user.email).toBe(payload.user.email);
        expect(res.body.user.username).toBe(payload.user.username);
    ...
    describe('email', function () {
      it('should return error if empty', function (done) {
        ...
        (0, _supertest2.default)(_app2.default).post('/api/users/login').send(payload).expect(422).end(function (err, res) {
          var data = JSON.parse(res.text);
          expect(data.errors.email).toBe('Can\'t be blank');
          done();
        });
  ...
  describe('GET: /user', function () {
    ...
    it('should return the current JSON representation of the current user', function (done) {
      ...
      (0, _supertest2.default)(_app2.default).post('/api/users').send(payload).expect(200).end(function (err, res) {
        ...
        (0, _supertest2.default)(_app2.default).get('/api/user').set('Authorization', token).expect(200).end(function (err, res) {
          expect(res.body.user.username).toBe(payload.user.username);
          expect(res.body.user.email).toBe(payload.user.email);
          expect(res.body.user.token.length).toBe(195);
          done();
    ...
    it('should return error if no token passed in header', function (done) {
      ...
      (0, _supertest2.default)(_app2.default).post('/api/users').send(payload).expect(200).end(function (err, res) {
        (0, _supertest2.default)(_app2.default).get('/api/user').expect(401).end(function (err, res) {
          var data = JSON.parse(res.text);
    ...
    it('should return error if token is wrong', function (done) {
      ...
      (0, _supertest2.default)(_app2.default).post('/api/users').send(payload).expect(200).end(function (err, res) {
        (0, _supertest2.default)(_app2.default).get('/api/user').set('Authorization', 'Token eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjVhNDM4MWU0NzY5ZDIxMjRhZjQ3NmFiMiIsInVzZXJuYW1lIjoiZm9vYmFyIiwiZXhwaXJhdGlvbiI6MCwiaWF0IjoxNTE0MzczNjA0fQ.UzDmWdo_F5uUQpvkado6oj99bhHHc1LSGgunWFkjRzI').expect(401).end(function (err, res) {
  ...
  describe('PUT: /user', function () {
    ...
    it('should modifiy the fields', function (done) {
      ...
      (0, _supertest2.default)(_app2.default).post('/api/users').send(payload).expect(200).end(function (err, res) {
        var token = 'Token ' + res.body.user.token;
        (0, _supertest2.default)(_app2.default).put('/api/user').set('Authorization', token).send(update).expect(200).end(function (err, res) {
    ...
    it('should return error if not loggued in', function (done) {
      (0, _supertest2.default)(_app2.default).put('/api/user').send(payload).expect(200).end(function (err, res) {
        (0, _supertest2.default)(_app2.default).get('/api/user').expect(401).end(function (err, res) {
          var data = JSON.parse(res.text);
          expect(data.errors.message).toBe('No authorization token was found');
          done();
